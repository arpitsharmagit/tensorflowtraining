...
def create_tf_record(output_filename,
                     ...
                     image_dir,
                     files):
  """Creates a TFRecord file from annotations.
  Args:
  ...
  for filename in files:        
    logging.info('annotation file %s',filename)
    path = os.path.join(annotations_dir, filename)

    if not os.path.exists(path):
      logging.warning('Could not find %s, ignoring annotation.', path)
      continue
...
def main(_):
  ...
  file_list = listdir(annotations_dir)
  ...
  train_examples = file_list[:num_train]
  val_examples = file_list[num_train:]
  logging.info('%d training and %d validation examples.',
               len(train_examples), len(val_examples))

  train_output_path = os.path.join(FLAGS.output_dir, 'object_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'object_val.record')
